...
const characterSet = [...upperCaseLetters.split(''), ...numbers.split('')];
...
function encode(data: any) {
  ...
  if (bytes === null) {
    const e = new Error(`Error: cannot encode data of type '${typeof data}'.`);
    throw e;
  }
...
function decode(data: any) {
  ...
  for (let i = 0; i < data.length; i += 8) {
    ...
    for (let b = 0; b < unpaddedBytes; b += 1) {
      buffer.push(Number((bits >> BigInt(8 * b + gap)) & BigInt(0b11111111)));
    }
