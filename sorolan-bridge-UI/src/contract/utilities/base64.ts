  ...
  if (bytes) {
    ...
    if (unpaddedLength > 0) {
      for (let i = 0; i < unpaddedLength; i += 3) {
        ...
        const bits = (bytes[i + 0] << 16) + (bytes[i + 1] << 8) + (bytes[i + 2] << 0);
    ...
    if (paddedBytes > 0) {
      ...
      if (paddedBytes === 1) {
        ...
        bits = (bytes[unpaddedLength + 0] << 10) + (bytes[unpaddedLength + 1] << 2);
...
function decode(data: any) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    if (data[i + 2] !== padding && data[i + 3] !== padding) {
      ...
      decodedData.push((bits >> 16) & 0b11111111, (bits >> 8) & 0b11111111);
    } else if (data[i + 2] === padding && data[i + 3] === padding) {
      // Section contains 2 padding characters.
      decodedData.push((bits >> 16) & 0b11111111);
    }
  }
